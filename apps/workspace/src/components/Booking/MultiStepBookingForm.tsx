...
const isValidEmail = (email: string): boolean => {
  const v = email.trim();
  if (!v) return false;
  // Reasonable client-side validation; backend may still enforce stricter rules.
  return /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v);
};
...
const isValidPhone = (phone: string): boolean => {
  ...
  // Loose validation: require at least 7 digits (supports intl/local formats).
...
const isValidDate = (value: string): boolean => {
  const v = value.trim();
  if (!v) return false;
  // HTML date inputs typically emit YYYY-MM-DD; Date.parse handles it.
...
const MultiStepBookingForm = ({ onSave, onCancel }: MultiStepBookingFormProps) => {
  ...
  // New state for department and service provider selection
  ...
  const sortedEventTypes = useMemo(() => {
    return [...eventTypes].sort((a, b) => {
      ...
      // Define the desired order: 15, 30, 45, then 60+
  ...
  const [workspacePrimaryColor, setWorkspacePrimaryColor] = useState<string>('#9333EA');
  ...
  const formatMinutesToDisplay = (minutes: number): string => {
    ...
    // Create a date with the time for consistent formatting
    const date = new Date();
  ...
  // Returns true if the slot starts before the current time and the date is today
  // This prevents users from booking slots that have already passed
  const isTimeSlotInPast = (slotStart: Date, checkDate: Date): boolean => {
    // Only check past times for today's date
  ...
  }, [selectedProvider, departments.length, loadingDepartments]);
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result.workspace) {
            ...
            if (result.workspace.logo_url) {
              setWorkspaceLogoUrl(result.workspace.logo_url);
            }
          }
        }
      } catch (error) {
  ...
  // Update colors from configurations table
  useEffect(() => {
    if (!loadingSettings && general) {
      if (general.primaryColor) {
        setWorkspacePrimaryColor(general.primaryColor);
      }
      if (general.accentColor) {
        setWorkspaceAccentColor(general.accentColor);
      }
    }
    ...
    const fetchBookingsForDateRange = async () => {
      ...
      try {
        ...
        const apiUrl = selectedProvider 
          ? `/api/bookings?start_date=${startDateStr}&end_date=${endDateStr}&service_provider_id=${selectedProvider.id}`
          : `/api/bookings?start_date=${startDateStr}&end_date=${endDateStr}`;
  ...
  }, [selectedProvider, selectedType, days, departments.length]);
  ...
  // Scroll to selected date in quick date picker
  useEffect(() => {
  ...
  }, [selectedDate, showCalendar, days]);
  ...
  // Reset per-field validation UI when (re)entering the intake step.
  useEffect(() => {
    if (step === 4) {
  ...
  useEffect(() => {
    ...
    const fetchServices = async () => {
      try {
        ...
        // If allowed list changed, ensure selection remains valid.
  ...
  const intakeValidation = useMemo(() => {
    ...
    // Built-ins: render only when enabled; validation only then.
    ...
    // Ensure config isn't impossible: name/email/phone all off breaks booking creation.
  ...
  const handleConfirm = async () => {
    ...
    if (!isStep4Valid) {
      const firstError = intakeValidation._config || intakeValidation.name || intakeValidation.email || intakeValidation.phone || intakeValidation.services;
    ...
    try {
      ...
      // Fetch workspace configuration to check auto-confirm setting
      ...
      // Final check: verify the time slot is not booked (double-check before API call)
      const isBooked = isTimeSlotBooked(startDate, endDate, selectedDate);
      if (isBooked) {
      ...
      if (additionalDescriptionEnabled && notes.trim()) {
        // Preserve existing email templates that read `metadata.notes`
        metadataPayload.notes = notes.trim();
      }
      if (Object.keys(intakeFormPayload).length > 0) {
        metadataPayload.intake_form = intakeFormPayload;
      }
  ...
  const formatTimeWithTimezone = (date: Date, timeString: string): string => {
    // Parse time string (format: "2:30 PM")
  ...
  const getServiceIcon = (duration: number) => {
    if (duration <= 30) {
      // Speech bubble icon for quick chats
      return (
      ...
      // Calendar icon for consultations
      return (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div className="w-full max-w-7xl h-auto mx-auto px-6 sm:px-4 py-4 sm:py-6 lg:py-8">
      <div className="rounded-xl drop-shadow-xl overflow-hidden bg-gray-100 relative backdrop-blur-xl">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <div className="absolute -top-40 -right-40 w-80 h-80 rounded-full opacity-10 blur-3xl" style={{ background: `radial-gradient(circle, ${workspacePrimaryColor || '#9333EA'}, transparent)` }}/>
          <div className="absolute -bottom-40 -left-40 w-80 h-80 rounded-full opacity-10 blur-3xl" style={{ background: `radial-gradient(circle, ${workspaceAccentColor || workspacePrimaryColor || '#3B82F6'}, transparent)` }}/>
        </div>
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-2 relative z-10">
          ...
          <div className="w-full lg:sticky lg:top-0 lg:overflow-y-auto bg-gradient-to-br p-4 sm:p-6 lg:p-8 relative overflow-hidden" style={{ background: `linear-gradient(135deg, $ {workspacePrimaryColor || '#9333EA'}08 0%, ${workspaceAccentColor || workspacePrimaryColor || '#3B82F6'}08 100%)` }}>
            ...
            <div className="absolute inset-0 opacity-5">
              <div className="absolute inset-0" style={{ backgroundImage: `radial-gradient(circle at 2px 2px, ${workspacePrimaryColor || '#9333EA'} 1px, transparent 0)`, backgroundSize: '24px 24px' }} />
            </div>
                  ...
                  <div className="py-2 z-10 relative">
                    <div className="flex flex-wrap items-center gap-4">
                      {!loadingSettings && workspaceLogoUrl ? (
                        isExternalLogoUrl ? (
                          <img src={workspaceLogoUrl} alt={workspaceName} className="w-12 h-12 rounded-xl object-contain" />
                        ) : (
                          <img src={workspaceLogoUrl} alt={workspaceName} className="w-12 h-12 rounded-xl object-cover" />
                        )
                      ) : (
                        <div className="w-12 h-12 rounded-xl flex items-center justify-center text-2xl text-white">
                          <img src="getsettime-icon.png" alt={workspaceName} className="w-12 h-12 rounded-xl object-cover" />
                        </div>
                      )}
                      <div>
                ...
                <div className="space-y-4 sm:space-y-6 animate-fadeIn">
                  ...
                  {loadingEventTypes ? (
                    ...
                    <div className="text-center py-16 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200">
                      ...
                      <p className="text-sm text-gray-400 mt-2">Please create an event type first</p>
                    </div>
                  ) : (
              ...
              {step === 3 && (
                <div className="space-y-4 sm:space-y-6 lg:space-y-8 animate-fadeIn">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 sm:mt-8 lg:mt-10 pt-6 sm:pt-8 border-t border-gray-200">
                    <button
                      onClick={() => {
                        // Go back to step 2 if no departments, otherwise step 1
                        ...
                        return (
                          <div key={field.id} className="group">
                            ...
                            <textarea
                              ...
                              onBlur={() =>
                                setTouchedCustomFields((prev) => ({ ...prev, [field.id]: true }))
                              }
                    ...
                    <div className="absolute inset-0 pointer-events-none">
                      {[...Array(12)].map((_, i) => (
                        <div
                          ...
                          style={{
                            background: ['#9333EA', '#3B82F6', '#10B981', '#F59E0B'][i % 4],
                            left: '50%',
                            top: '50%',
